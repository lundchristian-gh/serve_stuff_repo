...
void serve_client_impl(serve_stuff *self)
{
    ...
    if (strcmp(client_method, "POST") == 0)
        self->post_request(self, client_request);
    else if (file_exist)
    {
        if (strcmp(client_method, "GET") == 0)
            self->get_request(self, client_filename);
        else if (strcmp(client_method, "HEAD") == 0)
            self->head_request(self, client_filename);
    }
    else if (strcmp(client_filename, "") == 0)
        self->default_request(self);
    else
        self->not_found_request(self);
}
